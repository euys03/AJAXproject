	...
	<pre>
		...
		<script>
			...
			function jsFuncPost() {
				...
				var message = document.querySelector("#msg-1").value; // DOM 사용하여 입력받은 값 출력
				// 2. 요청 정보 설정
				xHttp.open("GET", "/ajax/first?msg=" + message, true); // 비동기로 동작 -> true
				// 3. 콜백 함수 설정 (응답했을 때 동작하는 함수)
				xHttp.onreadystatechange = function() {
					// ajax 상태, 서버 상태
					if(this.readyState == 4 && this.status == 200) {
						// status -> 200(요청성공), 404(page, url없음)
						// 500(서버 오류 발생), 403(접근거부), 400(쿼리스트링 오류)
						console.log("서버 전송 성공"); //Ajax의 목적 코드
					}else if(this.readyState == 4 && this.status == 404) {
						console.log("Page Not Found"); //Ajax의 목적 코드
					}
				}
				// 4. 전송 (어떠한 메시지를 보낼 때는 이러한 과정을 거쳐야 함 => 잘 쓰지 않음)
				xHttp.send();	// 객체이므로 String으로 전송
			}
			...
			function jqueryFunc() {
				...
				$.ajax({
					url : "/ajax/second",
					data : {"msg" : message},
					type : "get", // get메소드 = "get", post메소드 = "post"
					success : function() {
			...
			document.querySelector("#jq-btn3").addEventListener("click", function() {
				$.ajax({
					url : "/ajax/third",
					//data : {},	// data는 보낼 때 사용. 지금은 받는 것이므로 작성x
					type : "get",
			...
			// 4. 서버로 전송값 보내고 결과 문자열 받아서 처리 (송신-수신)
			$("#jq-btn4").on("click", function() {
				const num1 = $("#num-1").val();
				const num2 = $("#num-2").val();
				$.ajax({
					url : "/ajax/fourth",
					data : { "No1" : num1, "No2" : num2 },	// 키 값은 임의로 작성 가능, 값만 선언해준 것과 동일하게 작성
					type : "get",
			...
			// 5. 서버로 전송값 보내고 결과 JSON으로 받아서 처리
			...
			document.querySelector("#jq-btn6").addEventListener("click", function() {
				var userId = document.querySelector("#find-num").value;
				$.ajax({
					url : "/ajax/sixth",
					data : { "userId": userId },
					type : "get",
